    ...
    <section className="w-full bg-[#f8faf9] py-24 md:py-32 px-6 border-t border-earth-brown/10 relative z-20">
      <div className="max-w-container-max mx-auto">
        <div className="text-center max-w-3xl mx-auto mb-20">
          <span className="text-label-caps text-warm-gold mb-4 tracking-[0.2em] block uppercase">Reviews</span>
          <h2 className="text-display-lg text-primary mb-6">Verified Customer Experiences</h2>
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {reviews.map((rev, index) => (
            <motion.div
              ...
              className="bg-white p-8 rounded-2xl border border-earth-brown/5 shadow-sm flex flex-col justify-between text-left relative"
            >
              {/* Decorative quotation icon */}
              <span className="absolute top-6 right-8 text-6xl text-warm-gold/10 font-serif pointer-events-none">“</span>
              
              <div>
                <div className="flex items-center gap-1 mb-6">
                  {[...Array(rev.stars)].map((_, i) => (
                    <svg key={i} className="w-4 h-4 text-warm-gold fill-current" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                  ))}
                </div>
                <blockquote className="text-sm lg:text-body-md text-on-surface-variant leading-relaxed font-light mb-8 italic">
                  "{rev.quote}"
                </blockquote>
              </div>

              <div className="border-t border-earth-brown/10 pt-4 mt-auto">
                <span className="text-base font-semibold text-primary block">{rev.name}</span>
                <span className="text-[10px] text-warm-gold uppercase tracking-widest font-semibold">{rev.role}</span>
              </div>
            </motion.div>
          ))}
        </div>
